                ...
                >
                  {options?.room_types.map((rt) => (
                    <option key={rt.name} value={rt.name}>
                      {rt.room_type_name} · Rp {idr(rt.base_price)}/night
                    </option>
                  ))}
                </select>
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Meal plan">
                  ...
                  >
                    ...
                    {options?.meal_plans.map((mp) => (
                      <option key={mp.name} value={mp.name}>
                        {mp.label} (+Rp {idr(mp.price_per_adult)}/adult)
                      </option>
                    ))}
                  </select>
                </Field>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {options?.experiences.map((x) => {
                        ...
                        return (
                          ...
                          >
                            {x.experience_name} · Rp {idr(x.price)}
                          </button>
                        )
                      })}
                    </div>
                ...
                {quote ? (
                  <div className="space-y-2.5 text-[15px]">
                    <div className="flex justify-between text-zinc-600">
                      ...
                      <span>Rp {idr(quote.room_total)}</span>
                    </div>
                    {quote.meal_total > 0 && (
                      <div className="flex justify-between text-zinc-600">
                        <span>Meals</span>
                        <span>Rp {idr(quote.meal_total)}</span>
                      </div>
                    )}
                    {quote.discount > 0 && (
                      <div className="flex justify-between font-medium text-emerald-700">
                        <span>Voucher</span>
                        <span>−Rp {idr(quote.discount)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-zinc-600">
                      <span>PB1 {quote.tax_percent}%</span>
                      <span>Rp {idr(quote.tax_amount)}</span>
                    </div>
                    {moreQuotes.map((mq, i) =>
                      mq ? (
                        <div
                        ...
                        >
                          ...
                          <span>Rp {idr(mq.amount_after_tax)}</span>
                        </div>
                      ) : null,
                    )}
                    {(() => {
                      ...
                      return (
                        <>
                          {addonsGross > 0 && (
                            <div className="flex justify-between text-zinc-600">
                              <span>Add-ons (incl. PB1)</span>
                              <span>Rp {idr(addonsGross)}</span>
                            </div>
                          )}
                          ...
                          {pol && (
                            <p className="mt-2 border-t border-zinc-200 pt-2 text-xs leading-relaxed text-zinc-500">
                              ...
                              {pol.deposit_pct > 0 &&
                                ` Deposit expected now: Rp ${idr((grand * pol.deposit_pct) / 100)} (${pol.deposit_pct}%).`}
                            </p>
                          )}
                        </>
                      )
