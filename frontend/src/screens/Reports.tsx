  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        ...
        />
        <Stat label="ADR" value={`Rp ${idr(t?.adr ?? 0)}`} sub="room rate / room sold" />
        <Stat label="RevPAR" value={`Rp ${idr(t?.revpar ?? 0)}`} sub="room rev / room" />
        <Stat
          label="RevPAX"
          value={`Rp ${idr(t?.revpax ?? 0)}`}
          sub={`total spend / guest · ${t?.pax ?? 0} pax`}
        />
        <Stat
          label="Revenue (day)"
          value={`Rp ${idr(t?.total_revenue ?? 0)}`}
          sub={`room Rp ${idr(t?.room_revenue ?? 0)} · F&B Rp ${idr(t?.fnb_revenue ?? 0)} · other Rp ${idr(t?.other_revenue ?? 0)}`}
        />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-200 text-left text-xs uppercase tracking-wider text-zinc-500">
                  ...
                  <th className="py-1.5 pr-3 text-right">ADR Rp </th>
                  <th className="py-1.5 pr-3 text-right">RevPAR Rp </th>
                  <th className="py-1.5 text-right">Revenue Rp </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100">
                {d.trend.map((r) => (
                  <tr key={r.date}>
                    ...
                    <td className="py-1.5 pr-3 text-right">{idr(r.adr)}</td>
                    <td className="py-1.5 pr-3 text-right">{idr(r.revpar)}</td>
                    <td className="py-1.5 text-right">
                      {idr(r.room_revenue + r.fnb_revenue + r.other_revenue)}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-zinc-300 font-medium">
                  ...
                  <td className="py-2 pr-3 text-right">{idr(d.mtd.adr)}</td>
                  <td className="py-2 pr-3 text-right">{idr(d.mtd.revpar)}</td>
                  <td className="py-2 text-right">
                    {idr(d.mtd.room_revenue + d.mtd.fnb_revenue + d.mtd.other_revenue)}
                  </td>
                </tr>
              </tfoot>
            </table>
