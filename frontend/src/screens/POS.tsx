...
export default function POS() {
  ...
  const gstRate = Number(outletDoc?.gst_rate ?? 21)
                        ...
                        {group.map((t) => (
                          <button key={t.table}
                            ...
                            <div className="truncate text-[10px] opacity-70">
                              {t.bills > 0
                                ? <>Rp {idr(t.order_total)}{t.guests ? <> · <Users className="inline size-2.5" />{t.guests}</> : null}</>
                      ...
                      <div className="mt-2 space-y-1 rounded-xl bg-zinc-50 p-2">
                        ...
                        {t.orders.map((b) => (
                          <button key={b.order} onClick={() => openTab(b.order)}
                            ...
                            <span>{b.label}</span><span className="tabular-nums">Rp {idr(b.order_total)}</span>
                          </button>
                        ))}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {looseBills.map((o) => (
                      <button key={o.name} onClick={() => openTab(o.name)}
                        ...
                        {o.label} <span className="tabular-nums text-zinc-400">Rp {idr(o.order_total)}</span>
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <li key={r.name}>
                    <button disabled={!r.open} onClick={() => openTab(r.name)}
                      ...
                      <span className="flex shrink-0 items-center gap-1.5 tabular-nums">
                        <span className="font-semibold">Rp {idr(r.order_total)}</span>
          ...
          <div className="h-fit rounded-2xl border border-zinc-200 bg-white p-4 lg:sticky lg:top-4 lg:col-span-4">
                  ...
                  {detail.items.map((it) => (
                    <li key={it.row} className="group flex items-center justify-between gap-1">
                      ...
                      <span className="flex items-center gap-1">
                        <span className="tabular-nums">Rp {idr(it.amount)}</span>
                ...
                {cart.map((l) => (
                  <div key={l.menu_item} className="border-b border-zinc-100 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="w-14 text-right text-sm font-semibold tabular-nums">Rp {idr(l.qty * l.price)}</span>
            ...
            <div className="mt-3 space-y-1 border-t border-zinc-100 pt-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                {discOpen ? (
                  <span className="flex items-center gap-1">
                    <input autoFocus className={inputCls + " !w-24 !py-0.5 text-xs"} placeholder="Rp " inputMode="numeric"
                  ...
                  <button onClick={() => setDiscOpen(true)} className="flex items-center gap-1 text-xs font-medium text-brand-700 hover:underline">
                    ...
                    {(selected && detail ? detail.discount_amount : disc) > 0
                      ? `−Rp ${idr(selected && detail ? detail.discount_amount : disc)}`
                      : "Add discount"}
                  </button>
                )}
              </div>
              <div className="flex justify-between text-zinc-500"><span>Subtotal</span><span className="tabular-nums">Rp {idr2(taxable)}</span></div>
              <div className="flex justify-between text-xs text-zinc-400"><span>CPB1 ({gstRate / 2}%)</span><span className="tabular-nums">Rp {idr2(gstAmt / 2)}</span></div>
              <div className="flex justify-between text-xs text-zinc-400"><span>SPB1 ({gstRate / 2}%)</span><span className="tabular-nums">Rp {idr2(gstAmt / 2)}</span></div>
              <div className="flex justify-between text-base font-bold"><span>Total</span><span className="tabular-nums">Rp {idr2(grand)}</span></div>
            </div>
    ...
    <button onClick={onAdd}
      ...
      <div className="p-2.5">
        ...
        <div className="mt-0.5 flex items-center justify-between">
          <span className="text-sm font-semibold text-zinc-700">Rp {idr(it.price)}</span>
