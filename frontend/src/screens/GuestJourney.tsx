    ...
    <div>
      <div className="flex h-2.5 items-stretch gap-[3px]" aria-hidden>
        {ordered.map((r) => (
          <span
            key={r.name}
            title={`${r.check_in_date} → ${r.check_out_date} · ${r.status} · Rp ${idr(r.amount_after_tax)}`}
            className={cn(
              "rounded-sm",
          ...
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <div className="flex gap-6 text-right">
                ...
                <StatCell
                  label="Lifetime"
                  value={`Rp ${idr(stats.lifetime_value)}`}
                />
                <StatCell label="Per night" value={`Rp ${idr(avgNight)}`} />
              </div>
            </div>
          </div>
