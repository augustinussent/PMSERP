  ...
  const OrderCard = ({ o }: { o: LaundryOrder }) => (
    <li className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-3">
        ...
        {o.total > 0 && <span className="text-sm font-semibold tabular-nums">Rp {idr(o.total)}</span>}
      </div>
            ...
            {(board?.recent ?? []).map((o) => (
              <li key={o.name} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm">
                ...
                <span className="flex items-center gap-2">
                  <Badge tone={STATUS_TONE[o.status] || "zinc"}>{o.status}</Badge>
                  {o.total > 0 && <span className="tabular-nums font-medium">Rp {idr(o.total)}</span>}
                </span>
              </li>
            ))}
          </ul>
      ...
      {counting && (
        <Sheet onClose={() => setCounting(null)} title="Count the bag">
              ...
              return (
                <div key={r.name}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-zinc-400">{r.service_type} · Rp {idr(price)}</p>
                  </div>
          ...
          <button
            ...
            Collect {countPieces} piece{countPieces === 1 ? "" : "s"} · Rp {idr(countTotal)}
          </button>
        </Sheet>
      )}
      ...
      {returning && (
        <Sheet onClose={() => setReturning(null)} title={`Return to room ${returning.order.room_no}`}>
          ...
          <button
            ...
            Deliver & bill Rp {idr(returning.order.total)}
