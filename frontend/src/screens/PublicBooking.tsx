...
export default function PublicBooking() {
  ...
  useEffect(() => {
    ...
    document.title = p.meta_title || `${p.property_name}, ${p.city} - book direct from Rp ${idr(minPrice)}/night`
    setMetaTag(
      "description",
      p.meta_description ||
        (`${p.property_name} in ${p.city}: ${data.room_types.length} room types from Rp ${idr(minPrice)}/night. ` +
                      ...
                      {r?.quote ? (
                        <>
                          <p className="text-2xl font-semibold">
                            Rp {idr(r.quote.amount_after_tax)}
                        ...
                        <p className="text-sm text-zinc-400">
                          from Rp {idr(rt.base_price)}/night
                        </p>
                      )}
                    </div>
                    <Button
          ...
          {done ? (
            <div className="space-y-3">
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-emerald-800">
                ...
                <p className="mt-1 text-sm">
                  Total Rp {idr(done.amount)} - payable at the hotel. We've saved
            ...
            <div className="space-y-4">
              ...
              <label className="block">
                ...
                <input className={inputCls} value={form.phone} placeholder="+62 …"
                ...
                <select className={inputCls} value={form.meal_plan}
                  ...
                  {data.meal_plans.map((mp) => (
                    <option key={mp.name} value={mp.name}>
                      {mp.label} (+Rp {idr(mp.price_per_adult)}/adult/night)
                    </option>
                  ))}
                </select>
              </label>
                  ...
                  {(() => {
                    ...
                    return addonTotal > 0 ? (
                      <p className="mt-2 text-right text-xs text-zinc-500">
                        Experiences: +Rp {idr(addonTotal)} · added to your bill at
                        the hotel
                      </p>
                    ) : null
                  })()}
                  ...
                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-600">
                    {data.property.payment_mode === "Full online"
                      ...
                      : data.property.payment_mode === "Advance percent"
                        ...
                        : `A Rp ${idr(data.property.registration_fee)} registration fee is collected to confirm; the rest is paid at the hotel.`}
                  </div>
                )}
              {error && (
