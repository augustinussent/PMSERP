            ...
            <>
              {roomTypes.map((rt, i) => (
                <div key={i} className="flex flex-wrap items-end gap-2">
                  ...
                  <input className={cn(inputCls, "w-28")} type="number" placeholder="Rp /night"
            ...
            <>
              {mealPlans.map((mp, i) => (
                <div key={mp.code} className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-zinc-400">Rp /adult/night</span>
                </div>
              ))}
            </>
          )}
              ...
              <div className="flex flex-wrap gap-1.5">
                {roomTypes.filter((r) => r.code).map((r) => (
                  <Badge key={r.code} tone="zinc">
                    {r.name} Rp {r.base_price} ×{r.numbers.split(",").filter((x) => x.trim()).length}
                  </Badge>
                ))}
              ...
              <label className="block">
                <textarea
                  ...
                  placeholder={
                    ...
                    '"Rao, Asha",+62 98xxxx,Deluxe,25/12/2025,28/12/2025,2,Checked Out'
                  }
                  value={csv}
