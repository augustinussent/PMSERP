      ...
      {scope === "property" && prop && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Tile icon={IndianRupee} label="Revenue" value={`Rp ${idr(prop.revenue_today)}`}
              sub="today" />
            <Tile icon={Wallet} label="Collections" value={`Rp ${idr(prop.collections_today)}`}
              sub="today" />
          </div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
                {[
                  ...
                  ["Revenue", `Rp ${idr(prop.statistics.mtd_revenue)}`],
                  ["ADR", `Rp ${idr(prop.statistics.adr)}`],
                  ["RevPAR", `Rp ${idr(prop.statistics.revpar)}`],
                  ["Rooms sold", idr(prop.statistics.rooms_sold_mtd)],
                ].map(([k, v]) => (
                  <div key={k}>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-1.5 text-sm">
                <Row label="Collected today" value={`Rp ${idr(prop.finance.collections_today)}`} />
                <Row label="Outstanding" value={`Rp ${idr(prop.finance.outstanding)}`} tone={prop.finance.outstanding ? "text-amber-600" : undefined} />
      ...
      {scope === "portfolio" && port && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Tile icon={IndianRupee} label="Revenue" value={`Rp ${idr(port.totals.revenue_today)}`} sub="today" />
            <Tile icon={Wallet} label="Collections" value={`Rp ${idr(port.totals.collections_today)}`} sub="today" />
          </div>
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {port.properties.map((p) => (
                      <tr key={p.property}>
                        ...
                        <td className="py-2 pr-3 text-right tabular-nums">Rp {idr(p.revenue_today)}</td>
                        <td className="py-2 pr-3 text-right tabular-nums">Rp {idr(p.collections_today)}</td>
                        <td className="py-2 text-right tabular-nums">Rp {idr(p.outstanding)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
