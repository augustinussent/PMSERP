 ...
 *  localization pack (CPB1/SPB1 for India, TAX elsewhere). */
function columnsFor(format: string, d: ExportData): Col[] {
  ...
  if (format === "zoho")
    return [
      ...
      { header: "PB1 Identification Number (PB1IN)", pick: (r) => String(r.party_tax_id) },
  ...
  if (format === "tally")
    return [
      ...
      { header: "PB1IN", pick: (r) => String(r.party_tax_id) },
          ...
          {data && (
            <>
              <p className="mb-3 text-sm text-zinc-500">
                ...
                {data.totals.invoices === 1 ? "" : "s"} · taxable Rp 
                {data.totals.taxable.toLocaleString("id-ID")} · {data.tax_label} Rp 
                {data.totals.total_tax.toLocaleString("id-ID")} · total Rp 
                {data.totals.grand_total.toLocaleString("id-ID")}
              </p>
              {data.rows.length === 0 ? (
