 ...
 * property identity, PB1, privacy, booking page, payments, agent access. */
...
const STAY_TAX_SPECS: Spec[] = [
  ...
  { field: "gst_rate_low", label: "PB1 & Service %", type: "number" },
  {
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingsCard
        ...
        description="Check-in/out times, PB1 slabs and how long guest IDs are kept."
      ...
      <CardContent>
        ...
        {form && (
          <div className="mb-3 flex flex-wrap items-center gap-2 rounded-xl bg-zinc-50 p-2">
            ...
            <input className="w-24 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm" placeholder="Rate Rp " inputMode="numeric"
              value={form.rate} onChange={(e) => setForm({ ...form, rate: e.target.value.replace(/[^\d.]/g, "") })} />
            <input className="w-28 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm" placeholder="Express Rp  (opt)" inputMode="numeric"
            ...
            <tbody>
              {rates.map((r) => (
                <tr key={r.name} className="border-t border-zinc-100">
                  ...
                  <td className="text-right tabular-nums">Rp {r.rate.toLocaleString("id-ID")}</td>
                  <td className="text-right tabular-nums text-zinc-500">Rp {r.express_rate.toLocaleString("id-ID")}</td>
      ...
      <CardContent>
        ...
        {tiers.length === 0 ? (
          <p className="py-3 text-sm text-zinc-400">
            No tiers yet — e.g. "at 80% occupancy, +15% premium, minimum Rp 6,000".
          </p>
        ) : (
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody>
              {tiers.map((t) => (
                <tr key={t.name} className="border-t border-zinc-100">
                  ...
                  <td className="text-right tabular-nums">{t.min_rate ? `Rp ${t.min_rate.toLocaleString("id-ID")}` : "—"}</td>
                  <td className="text-right">
                    <button className="text-xs font-medium text-brand-700 hover:underline"
