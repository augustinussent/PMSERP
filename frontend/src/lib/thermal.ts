...
export function laundryDocketHtml(o: {
  ...
  return `
    ...
    <table>${rows}</table>
    <div class="rule"></div>
    <div class="row b lg"><span>Total</span><span class="num">Rp ${idr(o.total)}</span></div>
...
export function billHtml(b: BillData) {
  ...
  const rows = b.items.map((it) =>
    ...
    `<td class="num" style="width:52px">${idr(it.rate)}</td>` +
    `<td class="num" style="width:62px">${idr(it.amount)}</td></tr>`).join("")
  const money = (l: string, v: number, cls = "") =>
    `<div class="row ${cls}"><span>${l}</span><span class="num">Rp ${idr(v)}</span></div>`
  return `
    ...
    ${money("Service Charge", b.cgst, "sm")}
    ${money("PB1", b.sgst, "sm")}
    <div class="rule"></div>
