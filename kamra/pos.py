...
def bill_data(order: str):
	...
	loc_ctx = pack_for(doc.property).invoice_context(frappe.get_cached_doc("Property", doc.property))
	return {
		...
		"cgst": round(gst_amount * float(loc_ctx["split"][0][1]), 2),
		"sgst": round(gst_amount * float(loc_ctx["split"][-1][1]), 2),
